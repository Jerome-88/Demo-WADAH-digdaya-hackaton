...
export default function KontrakFinalPage() {
  ...
  // Revisiting an already-active contract lands straight on the confirmation
  // view, not back at the payment step. The real escrow/chat workspace lives
  // on /jasa/proyek/:talentSlug (reached from the dashboard) — this page's
  // job ends at "contract signed, dana ditahan", not ongoing project management.
      ...
      <main className="max-w-[680px] mx-auto px-4 py-10 pb-16">
        ...
        {phase === 'active' && (
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col gap-4">
            ...
            <div className="rounded-2xl p-6 border-2" style={{ borderColor: ORANGE, background: '#fff7ee' }}>
              <div className="flex items-center gap-2 mb-2">
                <i className="fa-solid fa-lock text-sm" style={{ color: ORANGE }}></i>
                <span className="text-[11px] font-bold font-inter uppercase tracking-wide" style={{ color: ORANGE }}>Dana Ditahan Platform</span>
              </div>
              <div className="font-sora font-extrabold text-2xl mb-2" style={{ color: '#1a1a1a' }}>{formatRupiah(budget)}</div>
              <p className="text-sm font-inter text-gray-600">Ditahan WADAH — akan dilepas ke {talent.name.split(' ')[0]} setelah kamu terima hasil kerja bulan ini. Pantau progres & chat langsung dari dashboard proyekmu.</p>
            </div>

            <button onClick={() => navigate('/jasa')} className="w-full text-white font-bold py-3.5 rounded-full transition-all text-sm cursor-pointer border-0 hover:brightness-110" style={{ background: GREEN }}>
